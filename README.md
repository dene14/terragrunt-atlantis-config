...
| `--filter`                   | Path or enhanced glob expressions to the directory you want scope down the config for. Supports recursive matching with `**`, brace expansion with `{pattern1,pattern2}`, and other advanced patterns. Default is all files in root | ""                |
